...
# --- Ẩn toàn bộ logo, avatar GitHub, footer, header, link web ---
hide_streamlit_ui = """
<style>
/* Ẩn menu góc phải (3 chấm), header, footer, logo Streamlit, avatar GitHub */
...
/* Ẩn toàn bộ cụm “Made with Streamlit” và avatar GitHub ở góc dưới */
section[data-testid="stBottom"] {
    display: none !important;
}

/* Ẩn thêm phần tử Streamlit Cloud footer (logo + link web) */
a[href*="streamlit.io"] {
    display: none !important;
}

/* Ẩn toàn bộ phần tử có chứa ảnh GitHub hoặc logo Streamlit */
img[alt*="GitHub"],
img[alt*="streamlit"] {
    display: none !important;
}
...
st.sidebar.markdown("---")
section = st.sidebar.selectbox("Tuỳ chọn", ["Trang chủ", "Giới thiệu", "Liên hệ", "Giao dịch", "Thông tin cá nhân", "Lịch sử giao dịch"], index=0)
...
# ❌ Đừng đọc lại và st.dataframe(df_quan) lần nữa kẻo hiển thị toàn bộ bảng
   ...
   with st.expander("🔍 Kiểm tra kết nối Google Sheets"):
        ...
        try:
            gcp_info = st.secrets["gcp_service_account"]
            st.write("✅ Đang dùng service account từ secrets:", gcp_info.get("client_email", "(ẩn email)"))
        except Exception as e:
            st.warning(f"Không đọc được secrets: {e}")
        ...
        try:
            ...
            st.error(f"Không mở được Sheet: {e}")

        test_sheet = st.selectbox(
            "Chọn sheet để thử ghi",
...
if section == "Giao dịch":
    ...
    # token chống ghi trùng (khi rerun sau submit)
    ...
    # ====== CẤU HÌNH SHEET ĐÍCH ======
    SUBMIT_WS = "YCGD"   # ví dụ: "CCQ_Submissions_Staging"
    ...
    if submitted:
        # Chuẩn hoá chuỗi
        investor_name = (investor_name or "").strip()
        fund_pick     = (fund_pick or "").strip()
        ...
        if not investor_name or not fund_pick or amount_vnd <= 0:
            ...
            # Sinh 1 token duy nhất cho lần submit này để chống ghi trùng
            token = f"{investor_name}|{fund_pick}|{float(amount_vnd)}|{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            if token == st.session_state.get("last_buy_token"):
                st.info("Yêu cầu này đã được ghi nhận.")
            else:
                try:
                    # đảm bảo header tồn tại ở sheet đích
                    ensure_headers(
                        SUBMIT_WS,
